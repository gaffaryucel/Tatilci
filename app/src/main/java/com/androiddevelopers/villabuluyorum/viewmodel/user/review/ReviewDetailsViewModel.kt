...
@Inject constructor(
    private val firebaseRepo: FirebaseRepoInterFace, private val auth: FirebaseAuth
    ...
    fun getReservationById(reservationId: String) = viewModelScope.launch {
        ...
        firebaseRepo.getReservationById(reservationId)
            ...
            .addOnFailureListener { exception ->
                ...
                _reservationMessage.value = Resource.error(
                    "Belge alınamadı. Hata: $exception",
                    null
                )
            }
    }
    ...
    fun createReview(comment: String, rating: Int) {
        val review = makeReviewMode(
            comment,
            rating
        )
        _reviewMessage.value = Resource.loading(null)
        firebaseRepo.createReview(review)
            .addOnSuccessListener {
                _reviewMessage.value = Resource.success(null)
                val map = HashMap<String, Any?>()
                map["rated"] = true
                firebaseRepo.changeReservationRateStatus(
                    reservation.value?.reservationId.toString(),
                    map
                )
                    .addOnSuccessListener {
            ...
            .addOnFailureListener {
                _reviewMessage.value = Resource.error(
                    "Hata :",
                    null
                )
            }
    ...
    private fun makeReviewMode(
        comment: String? = null, rating: Int? = null
    ): ReviewModel {
        return ReviewModel(
            ...
            villaId = reservation.value?.villaId,
    ...
    private fun sendReviewNotification() {
        ...
        val message =
            "$userName, ${reservation.value?.startDate} - ${reservation.value?.endDate}  tarihleri arasındaki konaklaması için bir değerlendirme bıraktı"
